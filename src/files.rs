...
use crate::{error::LimpError, parser::load_from_deps, storage::JsonDependency, RunType};
...
pub fn storage_path(run_type: RunType) -> String {
    ...
    match std::env::consts::OS {
        "windows" => match run_type {
            RunType::Dev | RunType::Test => format!(".\\dev\\dependencies.json"),
            RunType::Release => format!(
                "C:\\Users\\{}\\AppData\\Roaming\\limp\\dependencies.json",
                &uname
            ),
            RunType::Custom(path) => path,
        },
        _ => match run_type {
            RunType::Dev | RunType::Test => format!("./dev/dependencies.json"),
            RunType::Release => format!("/home/{}/.config/limp/dependencies.json", &uname),
            RunType::Custom(path) => path,
        },
    }
}
